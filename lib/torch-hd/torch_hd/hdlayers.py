...
class NonLinearPermEncoder(nn.Module):
    def __init__(self, dim_in, N, D = 5000, dist = 'bernoulli', p = 0.5, mean = 0.0, std = 1.0,
...
class PermutationEncoder(nn.Module):
    ...
    def forward(self, x, y = None):
        ...
        for frame_id in range(x.shape[1]):#[0...511]
            x_frame = x[:, frame_id, :]  # (batch, 512)
            out = torch.matmul(x_frame, self.proj_matrix.detach()) # (batch, 10000)
            if self.quantize_encodings:
                if self.dist == 'bernoulli':
                    out = torch.sign(out)
                elif self.dist == 'normal':
